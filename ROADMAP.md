...
- P5-2 executor 曾於 `871ee29` 實作、`f8f5400` revert 回 ADR-007 proposal-only 契約；重啟條件：P2.5 gate 全綠 + allowlist、dirty-worktree check、timeout/cancel、audit receipt、L0/L1/L2 分級批准全數就位，並以獨立 ADR 驗收。
  ...
  ▶ ✅ 2026-09-01：**Windows CI 紅燈根因修復（agent dispatch 編碼契約）**——`core/agent_dispatch.py` 的父行程固定以 UTF-8 解碼子行程輸出（`_truncate()`），但 `build_subprocess_env()` 從未規定子行程要用什麼編碼輸出。Windows 的 Python 對 pipe 預設用 ANSI code page（cp1252/cp950），**agent CLI 一輸出中文就 `UnicodeEncodeError` → exit 1 → executor 把 receipt 記成 `failed`**；Linux/macOS 預設 UTF-8 所以完全看不出來。這使 `platform-matrix` 的兩個 windows leg 在三個 L2 測試上失敗（`test_agent_apply_plan`／`test_agent_dispatch_l2` 各以 `assert 'failed' == 'succeeded'` 收場），main 從 run #44 連紅到 #48。修法：`build_subprocess_env()` **強制覆寫** `PYTHONUTF8=1` 與 `PYTHONIOENCODING=utf-8`（覆寫而非預設值——父行程既然固定 UTF-8 解碼，繼承來的任何其他值都是錯的），兩者只決定編碼、不帶機密，ADR-008 的 env allowlist 安全邊界不變。這不只是測試假 CLI 的問題：真的在 Windows 上輸出繁中計畫的 agent CLI 會踩同一顆地雷。新增回歸測試 `test_subprocess_child_emits_utf8_even_under_a_legacy_ansi_locale`（以繼承的 `PYTHONIOENCODING=cp1252` 重現，在任何平台都會抓到；已驗證修正前失敗、修正後通過）。收據：分支 workflow_dispatch [run #49](https://github.com/dofliu/activityTracker/actions/runs/33528561300) 與合併後 main 的 [run #50](https://github.com/dofliu/activityTracker/actions/runs/33566553321) 皆 6/6 全綠（[PR #1](https://github.com/dofliu/activityTracker/pull/1)）。
  ▶ ✅ 2026-09-02：**CI 加 `pull_request` trigger**——`platform-matrix.yml` 原本只在 push to main 與手動 dispatch 觸發，**任何 PR 都沒有 CI**，跨平台問題只能在進了 main 之後才被發現（上一條的五次連紅正是這個模式的結果）。加上 `pull_request: branches: [main]` 後同一套六 leg 矩陣會在合併前跑；push trigger 仍只限 main，所以一個 PR 的每個 head commit 只有一次 run，不會重複。注意這個改動本身有雞生蛋問題：它要先進 main 才會對後續 PR 生效。
  下一步：待辦清單集中於 [docs/TODO.md](docs/TODO.md)——目前是 §0 的 CI trigger 待併入，加上使用者側 live 收據（全天 coverage ledger 為唯一擋 `release_ready` 的能力缺口）與可選的功能候選。交接資訊見 docs/NEXT_SESSION.md。
