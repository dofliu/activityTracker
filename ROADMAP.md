...
- ✅ D11 欠的前端那一半有安全網了：`scripts/dashboard_dom_lock.py` ＋ 12 張 DOM 快照（2026-09-19，§11.2；[ADR-029](docs/ADR-029-frontend-dom-lock.md)）。**`state.js` 改成注入本身還沒做**——這一輪交付的是「改了能證明沒壞」的能力，不是改動。
...
> §13 的減法清單（R0～R2）**已全部完成**，最後一條是 2026-09-19 的前端行為鎖（[ADR-029](docs/ADR-029-frontend-dom-lock.md)）。
> 這一節接著 §13.3 往下走；**待辦條目與完成判準一律以 [docs/TODO.md](docs/TODO.md) E 段為準**，這裡只寫「為什麼是這三件事、為什麼不是別的」。

### 14.1 這一輪的判斷

檢視當天的實測（乾淨容器、Python 3.11）：`pytest` **822 項全綠**（820 passed ＋ 2 conditional skip）、
74 個 contract test 模組、`python main.py verify` 22 項全部有判定、migration 18/18。
**程式面沒有壞掉的東西，也沒有還沒寫的東西**——`release_ready` 卡的仍然是使用者實機收據（A 段），
而 A 段只能在 Windows 實機產生，不是任何開發 session 做得完的事。
...
1. **E1 `omni demo` 示範資料集**（先做，因為它是後面兩項的前置）
   ...
   - 邊界（要先寫進 ADR）：示範資料**一律標成示範**、**只能寫進另開的家目錄**、絕不與真實資料同庫，
     且驗收中心與問候卡對示範資料的數字不得宣稱成實機收據——否則就是這個專案最不能犯的錯：拿假資料當證據。
...
- **D13（前端 `state.js` 改注入）降級**：[ADR-026](docs/ADR-026-frontend-modules.md) 承諾在 D11 改、[ADR-027](docs/ADR-027-injected-runtime-state.md) 明說沒兌現，
  所以它一直掛在 TODO 上。但它要修的缺陷——「49 個散落在 `app.js` 的模組層 `let`」——**D10 就已經修掉了**：
  現在是一個具名的 `state` 物件，49 個值在一個檔案裡一次讀得完。
  剩下的「可注入」在後端買到的是**測試隔離**（`new_runtime_state()` 換一份），
  前端沒有等價的單元測試環境可以花用它——唯一的安全網 [ADR-029](docs/ADR-029-frontend-dom-lock.md) 是 DOM 快照，
  它證明的是「改完沒壞」，不是「改完更好測」。**花十個檔案的改動換一個現在用不到的性質，不划算**，所以降級而不是刪除：
  哪天前端真的有了單元測試，或 `state.js` 真的變成第二個 god object，再回來做。理由記在這裡，不靜默消失。
...
- 動到邊界的先寫 ADR（E1 的示範資料邊界、E2 的套件邊界都算）。
