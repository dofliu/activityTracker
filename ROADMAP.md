...
- ✅ 2026-09-23：**推廣路線 E1——`omni demo` 核心實作**（safety fail-closed ＋ 灌資料 ＋ `demo_mode` 傳播；ADR-031 邊界落地，`docs/ADR-031-omni-demo-dataset.md` 未再改版）。新增 `core/demo_dataset.py`：`resolve_demo_target()` 對照三個受保護路徑（實機預設家目錄 `~/OmniContext`、原始碼 checkout、當下 `OMNICONTEXT_HOME`）fail-closed 拒絕；目標目錄已存在但找不到 `.omnicontext_demo_marker.json` 旗標檔（`core/runtime_paths.demo_marker_path`／`is_demo_home`）一律拒絕清空；旗標檔存在才允許「清空重灌」。**與 2026-09-22 那則設計草記的落差，如實記下**：草記原本傾向讓示範資料**走既有 parser／watcher** 進系統；實作時改為**直接寫入 SQLAlchemy model**（`GitActivityEvent`／`AIPromptEvent`／`FileActivityEvent`），理由是合成出四種 transcript 格式各自合法的原始檔案，會直接撞上 ADR-025 的 drift 偵測契約與各 parser 自己的邊界測試，維護成本遠高於固定內容的假資料集；`omni demo` 的目的是「看得到系統長什麼樣子」，不是驗證 ingestion 路徑，兩者混在一起會讓一份程式碼背兩個目的。資料集固定兩個虛構專案（`aurora-notes` 近兩週持續在動、`lighthouse-api` 前一週活躍近一週歸零），灌完後在**全新子行程**跑 `core.project_engine.refresh_project_states(force=True)`（`OMNICONTEXT_HOME` 指到示範家目錄）——理由是這個函式吃行程內單例 `get_db()`／`get_config()`，另開子行程比動這個 session 的單例私有狀態乾淨，行為也與真正啟動服務時完全一致。`demo_mode` 旗標一路標到三處：`GET /api/v1/health`（新欄位）、`core.acceptance.report.build_acceptance_report()`（`demo_mode=true` 時任何原本 `passed`／`attested` 的項目強制降級為 `needs_human` 並附加「示範模式」字句——ADR-031 唯一不可退讓的判準）、`core.secretary.greeting.build_greeting()`（新欄位，供前端／Telegram 顯示橫幅，本輪未動前端）。新增 `main.py demo`（`--home` 選填，預設 `~/.omnicontext-demo`）。**容器 E2E 實測**（非 pytest，真的啟動服務）：`python main.py demo --home /tmp/omni-demo-e2e` → 對實機路徑與非示範既有目錄各測一次 fail-closed（兩者皆正確拒絕且未動任何檔案）→ 重灌一次驗證冪等 → `OMNICONTEXT_HOME=/tmp/omni-demo-e2e python main.py run` 起服務：`GET /api/v1/health` 回 `demo_mode: true`；`GET /api/v1/secretary/home` 的 `focus` 正確挑到 `lighthouse-api`（被冷落訊號）、`resume` 正確歸戶到 `aurora-notes`（靠幫每個示範專案資料夾補一個空的 `.git/` 讓 `resolve_project_from_path` 認得出來，否則會被歸戶成示範家目錄自己的資料夾名）；`GET /api/v1/secretary/greeting` 的 `demo_mode: true`；`python main.py verify` 在示範家目錄上跑：`demo_mode: true`，22 項無一停在 `passed`／`attested`。**這一輪沒做到、留給下一塊**：`memory_pick`（「記得」面板）在全新示範家目錄上是空的——秘書記憶區只吃 `secretary_notes`，而灌資料只寫三張原始事件表，沒有跑過任何 L0（`daily_digest`／`morning_pack`）reduce 出觀察；`reports/handoffs/` 同理是空的（`handoff_active_projects` 沒有被觸發）。下一塊要做的是：決定「`omni demo` 灌完資料後要不要順便觸發一次 `daily_digest`＋`handoff_active_projects`」（兩者都是 L0 唯讀 template，透過 `core.scheduled_tasks`／`core.agent_dispatch` 呼叫，需要另外設計不依賴使用者 UI 點擊的呼叫路徑），以及前端的示範模式橫幅（`web/js` 讀 `health`／`greeting` 的 `demo_mode`）。**收據**：新增 `tests/test_omni_demo.py`（14 項）；`pytest` 845 項（843 passed ＋ 2 skipped；改動前 831 項＝829 passed＋2 skipped，全部新增測試通過、無回歸）；`python main.py verify`（非示範家目錄）判定與本輪之前相同，本輪未改任何既有驗收項目的機器判準，只在 `demo_mode=true` 時整段降級。
- ✅ 2026-09-23（第二輪）：**推廣路線 E1——收尾（`memory_pick`／`reports/handoffs/`／前端示範橫幅），E1 全部完成**。上一輪留的「一小塊」：`core/demo_dataset.py` 的 `seed_demo_home()` 在灌完假資料、另開子行程重算 `project_states` 之後，**同一個子行程**再依序跑一次 `core.activity_digest.build_daily_digest(days_back=1)` 與 `core.secretary.packs.build_active_handoffs(hours=24*7, max_projects=10)`（改名 `_refresh_project_states_in_subprocess` → `_run_post_seed_steps_in_subprocess`，回傳值從 `None` 改成把子行程 stdout 的 JSON 解析回來的 `dict`）。**刻意不呼叫 `morning_pack`**——它還會跑 repo 同步報告與 STATUS 過期草稿，這兩項對一個沒有真實 repo 的示範家目錄沒有意義，只挑對示範資料有意義的兩個 L0。`seed_demo_home()` 的回傳值新增 `memory` 欄位（`refresh_project_states=False` 時是 `None`，代表「沒跑」而不是「跑了沒東西」，跟現有「東西在不在」≠「東西有沒有內容」的教訓對齊）；`main.py demo` 的 CLI 輸出跟著印出寫入的記憶則數與 Handoff 檔名。前端補上 ADR-031 D2 要求的「頂欄一條顏色不同的橫幅」：`web/index.html` 在 topbar 與 tabs 之間新增 `#demo-mode-banner`（預設 `hidden`），`web/js/core/ui.js` 新增 `loadDemoBanner()`（開頁時查一次 `GET /api/v1/health` 的 `demo_mode`，不輪詢——一個 session 開著的家目錄中途不會示範／實機互換），`web/js/main.js` 掛進 `DOMContentLoaded`；新字串 `demo_mode_banner` 同步補兩份字典。**容器 E2E 實測**（真的啟動服務，非 pytest）：`omni demo` 輸出「每日工作誌：2026-09-22（寫入 2 則記憶區觀察）」＋「Handoff：1 份 → reports/handoffs/（aurora-notes）」；`GET /api/v1/secretary/home` 的 `memory_pick` 不再是空的（挑到 `daily_digest` 觀察）；`reports/handoffs/Handoff_aurora-notes_*.md` 確實落地；Playwright 截圖確認示範家目錄上橫幅顯示、**非**示範家目錄（`/tmp/omni-normal-home`）上橫幅維持隱藏。**意外發現、如實記下、本輪未修**：同一次容器 E2E 也照出一個既有缺陷——`core/manager.py` 的 `Manager.__init__`（第 80–83 行）無條件啟動全部四個採集器（`file_watcher`／`git_watcher`／`window_watcher`／`agent_log_watcher`），完全沒有 `is_demo_home()` 判斷；`OMNICONTEXT_HOME` 只決定資料庫與旗標檔放哪，不影響採集器要看哪裡——所以對示範家目錄跑 `main.py run` 時，`agent_log_watcher` 照常去掃本機真正的 Claude Code transcript，把**這個雲端 session 自己當下的系統提示詞**當成一筆真的 `ai_prompt_events` 寫進了示範資料庫（`ai_prompt_events.id=6`，`cwd=/home/user/activityTracker`，`project_tag=activityTracker`，`prompt_text` 開頭就是這次排程任務的系統提示詞），連帶讓 `project_states` 多出一個非示範的 `activityTracker` 專案、把秘書桌面「上次做到哪」換成真人資料，等於「拿來公開展示的示範資料庫」在使用者真的把它跑起來當儀表板看時會混進真實工作內容——方向與 ADR-031 D1 保護的「示範資料混進實機」正好相反，ADR 本身沒有回答這個反方向。**已經寫進 `docs/TODO.md` B7**，不在本輪動它：需要先決定的設計問題是「demo home 該不該讓 `main.py run` 一併停用全部採集器」還是「只警告、由使用者自己在示範家目錄不要真的開著採集器」，屬於要先過 ADR 的邊界問題，不是一行修補。**收據**：`pytest` 847 項（845 passed ＋ 2 skipped，較上一輪淨增 2 項新測試、無回歸）；`python main.py verify`（非示範家目錄）判定與本輪之前逐字相同。
- ✅ 2026-09-26：**推廣路線 E2——[ADR-032](docs/ADR-032-readonly-mcp-context-server.md) 唯讀 MCP Context Server 的邊界定稿（Accepted）**。本輪**只寫 ADR、零產品程式碼**。TODO E2 指名要回答的兩件事都給了量過的答案：**①模組路徑＝頂層 `mcpserver/`**——官方 SDK 裝出來的是一個頂層、名字就叫 `mcp` 的 package，而本專案一律從 checkout root 執行（`sys.path[0]` 是 repo 根目錄），所以 repo 裡只要有 `mcp/` 就會把 SDK 整個遮掉；實測同一支程式、同一組 `PYTHONPATH`，在含 `mcp/` 的目錄跑得到 `ModuleNotFoundError: No module named 'mcp.server'`、換個目錄跑就 `OK`——是「從 repo 根目錄跑會壞、`pip install` 之後跑會好」的那種只在開發環境現形的 bug，同一個陷阱還會讓 `find_spec("mcp")` 的可用性檢查永遠成立。否決的候選連理由一起記在 ADR：`core/mcp/`（把唯讀邊界塞進 `core/agent_executor.py` 的兄弟目錄，D5 讀起來自相矛盾）、`integrations/mcp/`（最省事，但 `integrations/` 的語意是對外呼叫的第三方 client，方向相反）。**②官方 MCP SDK 採用，但只以 `[mcp]` optional extra 進場**——量法是 `pip install --dry-run --report` 解析兩次依賴閉包：核心 48 個套件、核心＋`mcp` 59 個，差額 11 個（`mcp`／`mcp-types`／`jsonschema`／`jsonschema-specifications`／`referencing`／`rpds-py`／`attrs`／`pyjwt`／`python-multipart`／`sse-starlette`／`opentelemetry-api`），`pip install --no-deps --target` 實測磁碟 **8.8 MB**。走 extra 之後核心安裝仍是 48 個套件／176 MB，**R0 買來的一個位元組都沒還回去**，於是 TODO 拿來反對引入 SDK 的那個理由（體積）就不成立了。採用的決定性理由是協定漂移：MCP 至今五個版本（`2024-11-05`／`2025-03-26`／`2025-06-18`／`2025-11-25`／`2026-07-28`），而且 SDK 自己把前四個標成 `HANDSHAKE_PROTOCOL_VERSIONS`、最後一個標成 `MODERN_PROTOCOL_VERSIONS`（stateless per-request envelope）——不到兩年換過一次世代，這正是 [ADR-025](docs/ADR-025-transcript-parsers-and-drift.md)「自己維護的東西會 drift」的同一課。**不好看的代價照實記**：SDK 硬相依 `sse-starlette`（R0 的 B6 才因為「零 import」把它從 `pyproject.toml` 移除）、`python-multipart`、`pyjwt[crypto]`、`starlette`、`uvicorn`，全是 HTTP／SSE／OAuth 那一半，而 HTTP／SSE transport 正是本 ADR 明文不做的；另會抬高版本下限（`pydantic>=2.12.0` vs 我們的 `>=2.6.0`、`pywin32>=311` vs `>=306`），並帶進一個 telemetry 套件 `opentelemetry-api`。**這一輪最有價值的產出其實不是那兩題，是動工前的盤點**：照 REVIEW 的字面做，第一版會違反自己的 D1。五個「看起來唯讀、其實會寫」的陷阱逐條查證——(1) `Database.init_db()` 無條件呼叫 `upgrade_sqlite_database(...)`，**一個 tool 都還沒跑**就已經建目錄、寫備份檔、套 migration（`core/database.py:31`）；(2) `session_scope()` 結束一定 `commit()`（`core/database.py:77`）；(3) `get_active_projects_list()` 是寫入函式——它無條件呼叫 `refresh_project_states()`，後者對 `project_states` 做 UPSERT 外加一次 bulk DELETE（`core/project_engine.py:373`／`:320`／`:346`）；(4) **REVIEW 的唯讀判準抓不到第 3 條**：在資料庫副本上實跑 `refresh_project_states(force=True)`，`project_states` 列數維持不變，但該列 `updated_at` 由 `13:12:12.224822` 變成 `13:12:24.492723`——「所有資料表列數不變」會綠燈放行一次 UPSERT，所以 A25 的判準從**列數**加嚴成**每張表的 `(列數, 全表內容雜湊)`**，並補一條「對唯讀連線下 `INSERT` 要拋 `OperationalError`」；(5) `build_daily_digest()` 會寫 `secretary_notes` 觀察（`core/activity_digest.py:249-258`），所以 `omni_recent_digest` 只准讀已落地的觀察。另外查證出一組 REVIEW 內部互相矛盾的要求：§A.2 說 `omni_handoff` 回「既有 `core/handoff_engine` 產物」，§A.4 D4 說「輸出無本機絕對路徑」——而實跑本機資料庫產生的 2808 字 handoff Markdown 掃出 **4 個絕對路徑**（專案根目錄 1 個、`/root/.claude/**.jsonl` 逐字稿 3 個，在 macOS／Windows 上就是使用者名稱），同一份產物還帶 `prompt` 全文與 `response[:500]`。ADR 以 D4 為準，定下「不轉送 core 既有產物，一律白名單投影，正則掃描只當第二道防線」。其餘決策：MCP 程序自己開唯讀連線（`mode=ro` ＋ 每條連線 `PRAGMA query_only=ON`，兩者都實測過；也誠實記下 `query_only` 在同一條連線上關得掉、`mode=ro` 才是不可撤銷的那個）；tool call receipt 寫 `logs/mcp_receipts.jsonl` **不寫資料庫**，這樣 D1 才能用最強的形式成立（整個程序沒有一條可寫連線），而不是「唯讀，但自己的收據表除外」；設定只加兩個平的鍵 `mcp.enabled: false` 與 `mcp.metadata_only: false`（`tests/test_config_surface.py:219` 的設定行上限是 260、目前 252，整個區塊只有 7 行額度，D6「六層收三層」不是修辭）。**收據**：本輪不動產品程式碼，`pytest` 與 `python main.py verify` 的輸出與基底相同。實作分兩片在 TODO E3／E4，完成判準已按本 ADR 加嚴。
...
> **現況（2026-09-22）**：本節的三項已改由 **§14 承接**，待辦條目與完成判準一律以 [docs/TODO.md](docs/TODO.md) E 段為準。
> 順序當天重排過兩次：先依相依關係改為 demo 先行（下方第 2 項，設計已定案於 [ADR-031](docs/ADR-031-omni-demo-dataset.md)，實作尚未開始），
> 再依外部檢視在 demo 與套件之間插入三項唯讀 MCP 工作，成為 **E1 → E6**（見 §14.2）。下方 1／2／3 項分別對應現在的 **E5／E1／E6**。
...
花三層開關守住的東西從旁邊開一道門。第一版的安全契約（D1–D6）在 ADR-032 定稿後才動工——[ADR-032](docs/ADR-032-readonly-mcp-context-server.md) 已於 2026-09-26 定稿（Accepted），E3／E4 可以動工。
