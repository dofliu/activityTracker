...
- ✅ 2026-09-23（第三輪）：**推廣路線 E2——ADR-032 唯讀 MCP Context Server 定稿**（[ADR-032](docs/ADR-032-readonly-mcp-context-server.md) Accepted，本輪只寫 ADR，沒有新程式碼）。回答了 REVIEW-2026-09-22 明講沒回答的兩件事：**模組路徑**選定 top-level `mcp_server/`（與 `rag/`／`notifiers/`／`watchers/` 同一層級，不叫 `mcp/`——那個名字與 PyPI 官方 MCP SDK 的 import 名稱撞名，兩者同時在 `sys.path` 上時哪個生效是環境相依的靜默錯誤）；**stdio SDK 相依**定案為 optional extra `[project.optional-dependencies].mcp`，比照 `[rag]` 的既有心智模型，`mcp_server/availability.py` 複製 `rag/availability.py` 的 `find_spec` 判斷形狀，主服務與 `omni mcp --selftest`（直接呼叫 tool 函式本體、不經 stdio）都不需要裝這個 extra 才能跑，只有真的接上 Claude Code 這一步需要。六個 tool 逐一對應到既有函式的薄封裝（`omni_project_state`→`project_engine.get_active_projects_list`、`omni_handoff`→`handoff_engine.build_project_handoff`、`omni_open_loops`→`project_engine.get_open_loops_list`、`omni_work_sessions`→既有（專案×日）聚合、`omni_recent_digest`→`activity_digest.build_daily_digest`、`omni_search_history`→**刻意選 `semantic_index.semantic_search` 而不是 `ask_local_context`**——後者會呼叫 LLM 做摘要合成，MCP 第一版要求 retrieval-only，合成交給呼叫端 agent），不新增第二套查詢邏輯。D1–D6 安全契約逐條寫成機器可查的判準（唯讀＝selftest 前後全庫列數不變、預設關閉＝`mcp.enabled: false`、不轉發金鑰＝零 `secret_resolver` import、輸出無 secret 與絕對路徑＝正則掃描、與執行器零耦合＝比照 ADR-024／ADR-025 既有手法掃 import 擋 `core.agent_executor`／`core.agent_dispatch`／`core.secretary.scheduled_tasks`、receipt 不存 query 原文）。第一版只做 stdio、不做 HTTP／SSE（因此不動 ADR-001 的 loopback 邊界）；不做清單明訂零 write tool、不暴露 `rag/` 的文件知識庫（`omni_search_history` 查的是 ADR-023 定案的「一份活動記憶」，與文件切片是刻意分開的兩件事）。**收據**：`pytest` 847 項（845 passed ＋ 2 skipped，與本輪之前逐字相同——本輪未動任何產品程式碼）；`python main.py verify`（全新空白家目錄）判定與既有模式一致（仍卡 A1、A2，22 項判定邏輯未變）。**下一輪待辦**：TODO E3——依本 ADR 建立 `mcp_server/` 套件骨架與前兩個 tool。
...
2. **E2 ADR-032 唯讀 MCP Context Server**（動工前先定稿，**已於 2026-09-23 第三輪 Accepted**）
   ...
   - **已定案**（[ADR-032](docs/ADR-032-readonly-mcp-context-server.md)）：模組叫 `mcp_server/`（top-level，不掛在
     `core/` 底下）；SDK 相依做成 optional extra `[mcp]`；六個 tool 對應既有函式的表、D1–D6 機器可查判準、
     不做清單全部寫進該 ADR。**實作留給 E3／E4**。
...
- 動到邊界的先寫 ADR（E1 的示範資料邊界已定稿為 ADR-031；E2 的 MCP 邊界已定稿為 ADR-032；E5 的套件邊界待動工時再取號）。
