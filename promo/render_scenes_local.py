...
def render_scene(html: Path, out_mp4: Path, *, duration: float, fps: int,
    ...
    frames_root = out_mp4.parent / "_frames"
    frames_root.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="intro_frames_", dir=str(frames_root)) as td:
        frames = Path(td)
        ...
        _found = sorted(_glob.glob(str(frames / "*.png")))
        print(f"[debug] frames dir={frames} count={len(_found)} first={_found[:1]} exists={frames.exists()}", flush=True)
