...
> **Current status: Personal Alpha (v1.3.0a5 published as a GitHub pre-release).** Windows milestone WinRT Toast E2E, schema 15/15, formal package+database rollback, P3-2 through P3-5 Context Memory Alpha, collector runtime diagnostics, the P2.6 continuous coverage ledger, and the cross-platform CI matrix have passed; Extension 1.3.1 obtained a live PASS receipt for ChatGPT and Claude.ai on 2026-08-31 and the P2.7 background-task live acceptance passed for all three platforms. **The secretary has landed through [ADR-008](docs/ADR-008-gated-agent-executor.md) stage P5-R3 plus the write addendum** (LLM notes, L0/L1 whitelist actions, L2 dispatch of your local agent CLI to draft plans / apply an approved plan — all off by default), plus the morning briefing (P5-R4a), two-tier incremental summaries, and the assistant-home UI. Remaining gap: a real full-day coverage-ledger receipt (release_ready stays false).
