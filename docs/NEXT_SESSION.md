...
| 測試 | **62 個 contract test 模組、629 項**（628 passed + 1 skipped）。容器缺 xdg-open 時 `test_open_command_is_argv_not_shell_string` 會條件 skip 並標註原因，不是失敗。 |
...
| **方向（2026-09-16 起）** | **減法優先**：功能候選 C5／C6／C3 暫停；B5–B9 已完成；接下來依 ROADMAP §13.2 的 R0 剩餘 D1（RAG 改選用依賴）→ R1（合併 LLM client／活躍聚合、切 router）→ R2（向量記憶二選一、秘書四層化、前端模組化）進行。乾淨容器實測：venv 797 MB → 721 MB（B6 之後）、`pytest` 628 passed ＋ 1 skipped。 |
...
- **方向與取捨看 [ROADMAP.md](../ROADMAP.md) §13「架構整頓與推廣方向」**（2026-09-16）：§12 的三條功能候選路線（C5／C6／C3）**暫停**，先做減法與合併；每個 D 項的完成判準在 TODO D 段。**R0 剩 D1**——接手時從它開始最安全（零設計決策）。
