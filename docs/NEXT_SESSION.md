...
| 測試 | **76 個 contract test 模組、847 項**（845 passed + 2 conditional skip；裝了 `[rag]` extra 的完整跑法，本輪已跑過並確認，`--junitxml` 核對過 errors=0/failures=0）。**不裝 `[rag]` extra 的變體本輪沒有另外裝一份 venv 複測**——`tests/test_omni_demo.py` 沒有 import 任何 `rag.*`，理論上不受影響，但沒有實測收據，下一輪如果要確認就跑一次不裝 `[rag]` 的 `pytest`。容器缺 xdg-open 時 `test_open_command_is_argv_not_shell_string` 會條件 skip 並標註原因，不是失敗。 |
...
| 驗收中心 | `core/acceptance/` 五個模組（ADR-028）：**去查什麼**在 `readings.py`、**查到什麼就算什麼**在 `items.py` 的階梯（與規格並排，由上往下第一條成立的說了算，**順序就是語意**），`rules.py` 是狀態字彙與十行直譯器，`report.py` 是人工署名與 release gate。加一項＝寫一個 reading ＋ 加一列；`tests/test_acceptance_declarative.py` 會擋下「忘了 OTHERWISE」「用了表格外的狀態」「檔案超過 600 行」。改任何一項的判準前先想清楚：`evidence` 是**對外契約**（會出現在 API 與畫面），要給判準用的私有值放 `facts`（D12，2026-09-18）。 |
| 程序內狀態 | **只有一份住處**：`core/runtime_state.py`（`ConfirmStore`／`ApprovalState`／`ChatState`／`TtlCache`／`ProjectCache`，每個自帶自己的鎖）。碰狀態的函式都有 `state=`／`cache=`／`confirms=` 具名參數，不給就用 `runtime_state()` 的行程預設。**測試不要再寫重設鉤子**——`tests/conftest.py` 的 autouse fixture 每個測試換一份新的 `RuntimeState`；要驗「兩份互不影響」就 `new_runtime_state()` 直接傳進去。這些狀態只在記憶體，一次性碼只存雜湊，不得落庫、寫檔或進 log（D11，2026-09-17；ADR-027）。 |
...
| **方向（2026-09-26 更新）** | **架構整頓（ROADMAP §13，R0～R2）已全部完成**（2026-09-20）；功能候選 C5／C6／C3 維持暫停。**§14 推廣路線**（docs/TODO.md E 段，依 **E1 → E6**）進度：**E1 `omni demo` 全部完成**（`main.py demo`／`core/demo_dataset.py`，[ADR-031](ADR-031-omni-demo-dataset.md)）——fail-closed、旗標檔判定、`demo_mode` 傳播到 health／驗收中心／問候卡、`memory_pick`、`reports/handoffs/`、前端示範橫幅全部落地並有容器 E2E 收據，已從 TODO 移除見 ROADMAP §11.2。**E2 已於 2026-09-26 完成**（[ADR-032](ADR-032-readonly-mcp-context-server.md) Accepted，零產品程式碼）：模組路徑定為頂層 **`mcpserver/`**（實測證明 repo 裡的 `mcp/` 會遮掉官方 SDK）、官方 MCP SDK **採用但只進 `[mcp]` optional extra**（量到 11 個套件／8.8 MB，核心 176 MB 不動）、MCP 程序自己開唯讀連線不碰 `get_db()`、receipt 寫檔不寫庫、唯讀證明由「列數不變」加嚴成「內容雜湊不變」。**接手從 E3 開始**（依 ADR-032 實作骨架 ＋ `omni_project_state`／`omni_handoff` 兩個 tool）→ E4 其餘四個 tool ＋ 驗收 A23–A26 → E5 `agent-transcripts` 獨立套件（前置 D9／ADR-025 已完成）→ E6 `omni init` 自動偵測。**E 段做完才輪到 F 段**（[ROADMAP §15](../ROADMAP.md)，repo 管理強化：F1 commit 來源歸因／F2 把已寫好卻零呼叫者的跨 repo GitHub 總覽接上畫面／F3 語言欄位／F4 作品集摘要候選）。乾淨容器實測：核心安裝 **176 MB**（含 `[rag]` 約 720 MB）。**新發現待處理**：TODO B7——示範家目錄上 `main.py run` 會啟動真正的採集器，混進真實活動，尚未修。 |
...
| 跨專案分流訊號（PR／issue） | `core/secretary/signals.py`（`split_stale_github_signals`，預設 60 天上限；ADR-024 前是 `core/triage_signals.py`） | `test_triage_signals.py`（11） | [ADR-007 Addendum](ADR-007-proposal-only-secretary.md) |
| 秘書提案（proposal-only） | `core/secretary/aggregate.py`（`build_action_proposals`／`annotate_action_proposals`；ADR-024 前是 `core/proactive_secretary.py`＋`core/secretary_advisor.py`） | `test_proactive_secretary.py`、`test_secretary_advisor.py` | [ADR-007](ADR-007-proposal-only-secretary.md)、[ADR-024](ADR-024-secretary-layers.md) |
...
| 秘書記憶區（大腦） | `core/secretary/memory.py`（`add_note`／`memory_context`；ADR-024 前是 `core/secretary_memory.py`） | `test_secretary_memory.py`（20） | [ADR-012](ADR-012-secretary-memory.md)、[ADR-024](ADR-024-secretary-layers.md) |
...
| 模式感知提案（秘書用它記得的東西） | `core/activity_patterns.py`（活動矩陣、`no_daily_routine`／`neglected_active_project`、習慣加權）；接在 `core/secretary/aggregate.build_action_proposals` | `test_activity_patterns.py`（21） | [ADR-017](ADR-017-pattern-aware-proposals.md) |
| 宣告式個人檔案（優先／語氣） | `core/secretary/memory.py`（`優先：`／`語氣：` 指令解析、優先加分、語氣；ADR-024 前是 `core/secretary_profile.py`）；接在 `build_action_proposals`、`memory_context`、`build_greeting`；`GET /api/v1/secretary/profile` 唯讀 | `test_secretary_profile.py`（27） | [ADR-018](ADR-018-declared-profile.md)、[ADR-024](ADR-024-secretary-layers.md) |
| 秘書桌面（01 首頁） | `core/secretary/present.py`（`build_home`：焦點＝第一張關於你的工作的提案、記得＝四級順序挑一則、詳情計數；ADR-024 前是 `core/secretary_home.py`）；`GET /api/v1/secretary/home` 唯讀；前端 `renderHome`／`recordHomeLeave` | `test_secretary_home.py`（14） | [ADR-019](ADR-019-secretary-desk-home.md)、[ADR-024](ADR-024-secretary-layers.md) |
...
| 每日包與今日視圖 | `core/secretary/packs.py`（`build_morning_pack`；ADR-024 前是 `core/secretary_packs.py`） | `test_secretary_packs.py`（9） | [ADR-008](ADR-008-gated-agent-executor.md) L0、[ADR-024](ADR-024-secretary-layers.md) |
| 問候卡（01 首頁＋晨報開頭） | `core/secretary/greeting.py`（`build_greeting`；ADR-024 前是 `core/secretary_greeting.py`） | `test_secretary_greeting.py`（23）＋晨報三項 | ROADMAP §11（2026-09-04）、[ADR-024](ADR-024-secretary-layers.md) |
| 活動來源與（專案 × 日）聚合 | `core/activity_sources.py` | `test_activity_sources_single_source.py`（12） | ROADMAP §13 D3 |
| API 路由表 | `core/server.py`（組裝）＋ `core/api/*.py`（9 個領域 router）＋ `core/schemas.py`＋`core/ingest.py` | `test_api_route_snapshot.py`（3，鎖 133 條）、`test_api_boundary.py`（18） | ROADMAP §13 D4 |
...
| Telegram 對話與批准 | `notifiers/telegram_chat.py`、`notifiers/telegram_approvals.py`、`core/secretary/present.py`（`ask_secretary`；ADR-024 前是 `core/secretary_ask.py`） | `test_telegram_chat.py`（29） | [ADR-013](ADR-013-telegram-secretary-chat.md)、[ADR-024](ADR-024-secretary-layers.md) |
...
| `omni demo` 示範資料集 | `core/demo_dataset.py`（`seed_demo_home`／fail-closed／`_run_post_seed_steps_in_subprocess` 灌完資料後跑 `daily_digest`＋`handoff_active_projects`）、`core/runtime_paths.py`（`is_demo_home`／`demo_marker_path`）、`main.py demo`、前端 `#demo-mode-banner`（`web/js/core/ui.js` 的 `loadDemoBanner`） | `test_omni_demo.py`（16） | [ADR-031](ADR-031-omni-demo-dataset.md)；**全部完成**（TODO E1）。已知缺陷見 TODO B7（`main.py run` 對示範家目錄一樣會啟動真正的採集器） |
...
- **新訊號就加一個 collector，不要改引擎核心**：`build_action_proposals` 的骨架是「收集 signals → snooze/mute → 排序 → diversity」。模式提案（ADR-017）只是多一個 `collect_*_signals` 與一段加權，形狀與其他 signal 一致就自動享有 snooze／mute／上限。
- **「偏好」要能改變行為，否則只是注入的文字**：ADR-012 的偏好筆記原本只有 `mute:` 是真的。新的個人化需求先問「使用者能不能直接寫下來」——能就做成明確指令（ADR-018 的 `優先：`／`語氣：`），從偏好筆記解析、不開第二套資料、不從活動或 prompt 推斷；宣告的權重要壓過推出來的（0.2 > 0.15）。
- **首頁的焦點是使用者的事，不是工具的事**：分數最高的提案不一定該上首頁——`verify_extension_heartbeat` 永遠 1.0 分，若不排除會永久占住焦點（ADR-019 容器 E2E 第一輪就看到）。任何「挑一張給使用者」的規則都先問：這張講的是他的工作，還是 OmniContext 自己的設定？
...
- **大搬家先做快照，再動刀**：D4 要把 1,995 行的 `core/server.py` 拆成 9 個 router，唯一真正危險的是「某條路徑悄悄消失或改名」——前端 97 條呼叫、Extension、Telegram 都吃字面。做法是先寫一支從執行中的 app 抓下全部 133 條（路徑、方法、handler 名稱）的快照測試並確認它在**搬家前**是綠的，然後才切。搬家本身用 AST 切、import 用「實際用到的名稱」推導、pyflakes 把關零未定義零未使用。另一個必然的連帶：`monkeypatch.setattr("core.server.X")` 會全部失效，因為 X 已經在別的模組——14 處 patch 目標要跟著搬（只改 import 路徑，斷言一字不動），這是模組切分無法避免的代價，不是測試寫壞了（2026-09-16）。
- **「重複」要先核對再動手，說法可能已經過期**：D3 的原始判斷是「四個模組各自算一遍每專案每日活躍」，實際查下去發現其中兩組早就互相委派了（weekly_review → activity_patterns、activity_digest → secretary_greeting）。真正的重複是**三件事各寫三遍**：哪些表算活動、專案名在哪個欄位、一天從哪到哪。照原始說法硬合併回傳值會做錯事；先核對才找得到真正該收的東西（2026-09-16）。
...
- **`OMNICONTEXT_HOME` 只決定資料存哪，不決定要看什麼**：`core/manager.py` 的 `Manager.__init__` 無條件啟動全部四個採集器，完全不看 `is_demo_home()`——所以對 `omni demo` 建的示範家目錄跑 `python main.py run`，`agent_log_watcher` 照樣去掃本機真正的 Claude Code transcript，把當下這個 session 自己的系統提示詞當成一筆真的 `ai_prompt_events` 寫進示範資料庫（2026-09-23 第二輪容器 E2E 親眼看到：`id=6`、`cwd=/home/user/activityTracker`）。任何「另開一個家目錄」的功能都要分開問兩件事：**資料存哪**（`OMNICONTEXT_HOME` 已經管了）與**要不要監看真實世界**（目前沒有任何開關管，見 TODO B7）。
...
- **文件同步**：功能落地時一次補齊下表；**每份文件只有一個職責，不要在第二個地方再寫一次**（2026-09-13 整理時，README 有七項功能沒補上、設定與安裝步驟在三個地方各寫一份且互相矛盾，就是沒守這條的代價）。
  ...
  新增 ADR 時同步更新 [INDEX.md](INDEX.md) 的一覽表；改 TODO A 段的判準時同步改 `core/acceptance/items.py` 的 `ITEMS`。
