...
5. **Telegram live 驗收**:使用者在「07 監控配置 → Telegram 通知」卡片走完設定流程(BotFather 建 bot → 貼 token → 偵測 chat id → 測試訊息送達 → 儲存啟用),再開「inline 批准」+「🔓 解鎖遠端批准」,實批一次 L1 動作取得 approved_via=telegram_inline receipt。
