...
| [ADR-022](ADR-022-meeting-secretary.md) | 會議秘書（Proposed：先接會後逐字稿，不碰即時音訊） | 「在開會」只用行事曆＋前景視窗兩個確定性訊號、逐字稿只從一個資料夾走既有索引路徑（需 WebVTT parser）、摘要預設本機 LLM 且雲端要明示、候選待辦不自動成為未結事項、即時音訊另案並寫下五道門 |
