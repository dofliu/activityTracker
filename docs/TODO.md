...
> 最後更新：2026-09-23 第二輪（**E1 `omni demo` 全部完成**，已移出本頁見 [ROADMAP §11.2](../ROADMAP.md)；接手從 **E2** 開始；新增 **B7**——示範家目錄跑 `main.py run` 會啟動真正的採集器，混進真實活動）。
...
> **E1 已於 2026-09-23（第二輪）全部完成**（`omni demo` 核心＋`memory_pick`／`reports/handoffs/`／前端示範橫幅），**E2 已於 2026-09-26 完成**（[ADR-032](ADR-032-readonly-mcp-context-server.md) Accepted：模組路徑定為頂層 `mcpserver/`、官方 MCP SDK 以 `[mcp]` optional extra 進場、六個 tool 的 schema、D1–D6 改寫成可執行的形狀），紀錄見 [ROADMAP §11.2](../ROADMAP.md)。**接手從 E3 開始**。過程中發現的一個既有缺陷（demo home 上 `main.py run` 會啟動真正的採集器）記在下面 B7，不在 E 段裡。

| # | 項目 | 內容 | 完成判準（收據） | 階段 |
| :-- | :--- | :--- | :--- | :--- |
| E3 | MCP 第一版（切片一）：骨架 ＋ 兩個 tool（邊界見 [ADR-032](ADR-032-readonly-mcp-context-server.md)） | `omni mcp` 子指令 ＋ `omni_project_state`、`omni_handoff` 兩個 tool（直接讀 SQLite，**不要求主服務在跑**）。先做這兩個，是因為它們只接既有的 `core/project_engine`／`core/handoff_engine`，不碰檢索層。 | `omni mcp --selftest` 兩個 tool 各回一次、每筆帶 `source_ref` 且回查得到 SQLite row；**唯讀證明（[ADR-032](ADR-032-readonly-mcp-context-server.md) 加嚴）**：selftest 前後每張表的 **`(列數, 全表內容雜湊)` 都不變**（真 tmp DB）——光比列數不夠，ADR-032 的 Context 陷阱 4 已實測證明 UPSERT 會讓列數不動而內容改變；另加「對 `readers.py` 的連線下一次 `INSERT` 要拋 `OperationalError`」與 **parity 測試**（同一份 fixture DB 上 `core/` 函式與 `mcpserver/` reader 的重疊欄位相等——D1–D6 沒有一條在證「答案是對的」，見 ADR-032 Consequences）；契約測試守門「`mcpserver/` 不得 import `core.agent_executor`／`core.agent_dispatch`／`core.secretary.scheduled_tasks`／`core.database`」（掃**直接** import，不掃遞移閉包）與「原始碼零寫入關鍵字」與「repo 根目錄不得有 `mcp/`」與「只有 `server.py` 能 import 官方 SDK」；`mcp.enabled: false` 時拒絕啟動並說出原因；輸出不含 token／secret／絕對路徑（正則掃描）；`pytest` 全綠且不裝 `[rag]` 亦全綠 | 推廣 |
| E4 | MCP 第一版（切片二）：其餘四個 tool ＋ 驗收（邊界見 [ADR-032](ADR-032-readonly-mcp-context-server.md)） | `omni_search_history`（**retrieval-only，不做 LLM 合成**——合成交給呼叫端 agent，server 只負責「找得到、指得回」）、`omni_open_loops`、`omni_work_sessions`、`omni_recent_digest`；並把驗收中心加到 A23–A26。 | 四個 tool 的回傳 schema 逐鍵比對（比照 [ADR-024](ADR-024-secretary-layers.md) `to_dict()` 的做法）；Ollama 不可用時 `omni_search_history` 回明確 `unavailable`，**不** fallback 到雲端、也不用空陣列冒充「沒發生」；`omni_recent_digest` 對沒有觀察的日期回「無觀察」而非即時產生；tool call receipt 存在且不含 query 原文；A23／A25／A26 機器可查、A24 標 `needs_human`，四項同步進 `core/acceptance/items.py` 的 `ITEMS` 與本頁 A 段；`python -m build` ＋ `verify_release_artifacts.py` `status: passed` 且 wheel 含 `mcpserver/` 全部模組 | 推廣 |
