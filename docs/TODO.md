...
> **R0 已於 2026-09-16 完成**（B5–B9 ＋ D1）、**R1 全部於同日完成**（D2 一個 LLM client、D3 一份活動來源定義、D4 `server.py` 切成 9 個 router、D5 桌面通知併入 `ChannelAdapter`、D6 旗標六層收三層），收據見 ROADMAP §11.2；R2 已完成 D7（一份活動記憶，ADR-023）與 D8（秘書四層化，ADR-024）；剩 D9–D12，每項都要先寫 ADR。
