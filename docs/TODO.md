...
> 最後更新：2026-09-23 第三輪（**E2 ADR-032（唯讀 MCP Context Server）定稿**，已移出本頁見 [ROADMAP §11.2](../ROADMAP.md)；接手從 **E3**（MCP 第一版切片一：`mcp_server/` 骨架 ＋ `omni_project_state`／`omni_handoff` 兩個 tool）開始）。
> 2026-09-23 第二輪：**E1 `omni demo` 全部完成**，已移出本頁見 [ROADMAP §11.2](../ROADMAP.md)；新增 **B7**——示範家目錄跑 `main.py run` 會啟動真正的採集器，混進真實活動。
...
| B7 | **`main.py run` 對示範家目錄一樣會啟動真正的採集器，示範資料庫會混進真實活動** | `core/manager.py:80-83`（`Manager.__init__`）無條件建立並啟動全部四個採集器（`file_watcher`／`git_watcher`／`window_watcher`／`agent_log_watcher`），沒有任何 `core.runtime_paths.is_demo_home()` 判斷——`OMNICONTEXT_HOME` 只決定資料庫與旗標檔放在哪，不影響採集器要監看哪裡（`agent_log_watcher` 預設就是掃本機真正的 Claude Code／Codex transcript 路徑）。**2026-09-23（第二輪）容器 E2E 實測到**：對 `/tmp/omni-demo-e2e`（`omni demo` 建立的示範家目錄）跑 `python main.py run` 後，`agent_log_watcher` 把**這個雲端 session 自己當下的系統提示詞**寫成一筆真的 `ai_prompt_events`（`id=6`、`cwd=/home/user/activityTracker`、`project_tag=activityTracker`），連帶讓 `project_states` 多出一個非示範的 `activityTracker` 專案，把秘書桌面「上次做到哪」換成真人資料。方向與 [ADR-031](ADR-031-omni-demo-dataset.md) D1 保護的「示範資料混進實機」正好相反：這裡是**實機活動混進打算公開展示的示範資料庫**，ADR-031 沒有回答這個反方向 | 需要先做設計決策、不是一行修補：選項 (a) `main.py run` 偵測 `is_demo_home()` 時直接不啟動任何採集器（讓示範家目錄永遠是唯讀展示，`omni demo --home` 重灌才能更新內容）；(b) 啟動但顯眼警告（風險：使用者不一定會注意到，且警告不能阻止資料已經寫入）。傾向 (a)，但要先確認没有既有流程依賴「示範家目錄也能繼續採集」；動到 `core/manager.py` 啟動路徑屬於邊界改動，先寫 ADR（可掛在 ADR-031 的 Addendum，或另開一份） | 🟡 P1 |
...
> **E1 已於 2026-09-23（第二輪）全部完成**（`omni demo` 核心＋`memory_pick`／`reports/handoffs/`／前端示範橫幅），紀錄見 [ROADMAP §11.2](../ROADMAP.md)。過程中發現的一個既有缺陷（demo home 上 `main.py run` 會啟動真正的採集器）記在下面 B7，不在 E 段裡。
> **E2 已於 2026-09-23（第三輪）定稿**（[ADR-032](ADR-032-readonly-mcp-context-server.md) Accepted：模組叫 `mcp_server/`、SDK 相依走 optional extra `[mcp]`、六個 tool 對應表、D1–D6 判準、不做清單全部定案），紀錄見 [ROADMAP §11.2](../ROADMAP.md)。**接手從 E3 開始**（骨架 ＋ 前兩個 tool 的實作）。

| # | 項目 | 內容 | 完成判準（收據） | 階段 |
| :-- | :--- | :--- | :--- | :--- |
| E3 | MCP 第一版（切片一）：骨架 ＋ 兩個 tool | 依 [ADR-032](ADR-032-readonly-mcp-context-server.md) 新增 top-level `mcp_server/` 套件（`pyproject.toml` packages.find.include 加 `"mcp_server*"`、optional extra `[mcp]`、`mcp_server/availability.py` 比照 `rag/availability.py`）＋ `omni mcp` 子指令 ＋ `omni_project_state`、`omni_handoff` 兩個 tool（封裝 `core.project_engine.get_active_projects_list`／`core.handoff_engine.build_project_handoff`，直接讀 SQLite，**不要求主服務在跑**）。先做這兩個，是因為它們只接既有的 `core/project_engine`／`core/handoff_engine`，不碰檢索層。 | `omni mcp --selftest` 兩個 tool 各回一次、每筆帶 `source_ref` 且回查得到 SQLite row；**唯讀證明**：selftest 前後所有資料表列數不變（真 tmp DB，ADR-032 D1）；契約測試守門「`mcp_server/` 不得 import `core.agent_executor`／`core.agent_dispatch`／`core.secretary.scheduled_tasks`」（D5）與「原始碼零寫入關鍵字、零 `secret_resolver` import」（D3）；`mcp.enabled: false` 時拒絕啟動並說出原因（D2，`config.example.yaml` 新增 `mcp:` 區塊）；輸出不含 token／secret／絕對路徑（正則掃描，D4）；tool call receipt 不存 query 原文（D6）；`pytest` 全綠且不裝 `[rag]`／不裝 `[mcp]` 亦全綠 | 推廣 |
