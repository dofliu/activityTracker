...
> **E1 已於 2026-09-23（第二輪）全部完成**（`omni demo` 核心＋`memory_pick`／`reports/handoffs/`／前端示範橫幅），紀錄見 [ROADMAP §11.2](../ROADMAP.md)。**接手從 E2 開始**。過程中發現的一個既有缺陷（demo home 上 `main.py run` 會啟動真正的採集器）記在下面 B7，不在 E 段裡。
