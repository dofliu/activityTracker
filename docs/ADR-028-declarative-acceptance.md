...
**差分收據**：把改動前的 `core/acceptance.py` 原封不動載成第二個模組，用 **85 個合成狀態**
（每一項的每一條分支各至少一次，含 runtime、停用、失敗、正在進行中、舊索引、空的 day token、
字串當路徑、`result_json` 不是物件…）同時餵給改動前與改動後，比對整份報告的 JSON
——**鍵的順序也算**。85 個全部逐位元組相同。

harness 自己也驗過會壞：蓄意把 `:.2%` 改成 `:.1%`、把 A21 的兩個判斷對調、把 `or not result`
拿掉，三種都被抓出來。

**那份 harness 不會留在 repo 裡**（它需要改動前的檔案才跑得動），所以它留下了一份常駐的替身：
`tests/test_acceptance_declarative.py` 末尾四支「順序就是語意」的回歸測試。挑的那四組不是隨便挑的
——A15 的開關在最後、A16 的在最前、A21 的「沒跑完」先於「孤兒目錄」、「進行中」先於舊的完成收據
——**把任一組對調，D12 之前就存在的 36 項測試全部仍然是綠的**，只有這四支會紅。實測確認過。
