...
> **2026-09-16 更新（[ADR-023](ADR-023-one-activity-memory.md)）**：筆記與時段微摘要不再進 RAG——活動記憶收回 `core/semantic_index`，每筆只 embedding 一次；`activity_sync` job 與 `POST /api/v1/rag/memory/sync` 保留，但只同步秘書寫出的**報告檔**，並清掉舊版重複的活動切片。對話引用得到的東西不變。
