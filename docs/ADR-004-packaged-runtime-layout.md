...
7. Build success 不等於 release gate 通過；必須另驗證 artifact contents、fresh install、上一版 wheel upgrade、application-home boundary、schema 4/4 與 Dashboard endpoints。
...
- `1.3.0a1` 仍是 Personal Alpha；Windows isolated smoke 通過不等於 macOS/Linux 或真實 Browser ingestion 已驗證。

## Verification Evidence

- 修改前 `1.2.0` wheel：config template、Web index、Extension Monitor、Extension manifest 全部缺失。
- `1.3.0a1` wheel/sdist：必要 assets、CLI entry point 與 privacy exclusions 通過 content receipt。
- Fresh install：config/database 均位於隔離 application home，schema `4/4`，health/dashboard/monitor/static endpoints 均為 HTTP 200。
- Upgrade install：`1.2.0 → 1.3.0a1` pip replacement、相同 runtime checks 與 schema `4/4` 通過。
