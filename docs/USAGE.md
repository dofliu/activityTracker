...
> 適用版本：`1.3.0a5` Personal Alpha
>
> 主要驗證平台：Windows 11、Python 3.12、Chrome/Edge MV3

本文件是**操作手冊**——可直接照做的安裝、啟動、配對、日常使用、備份與故障排查流程。
**「為什麼這樣設計、邊界在哪裡」不在這裡**：每個功能的決策見對應 ADR（一覽表在 [docs/INDEX.md](INDEX.md)），
做過什麼見 [ROADMAP.md](../ROADMAP.md) §11，還沒拿到的實機收據見 [TODO.md](TODO.md)。
...
5. [小秘書：桌面、提案與記憶](#5-小秘書桌面提案與記憶) — 01 分頁由上往下
6. [行事曆與會議](#6-行事曆與會議) — 本機 `.ics`、會後逐字稿
...
> 01 分頁是每天的起點。這一節照「打開後由上往下」的順序寫。

### 秘書桌面：01 是首頁，其他分頁是詳情（ADR-019）

打開 01，問候卡下面就是**秘書桌面**。它不是新的資料，而是秘書從既有的東西裡**挑出現在該看的**：

- **上次做到哪**（含接續／複製 Handoff／開資料夾）——原本在提案清單最上面，搬到桌面。
- **焦點 · 現在最該看的一件事**：提案引擎排序後第一張**關於你的工作**的提案（分數已含你宣告的優先與習慣加權），附「為什麼是現在」、批准按鈕（執行器開著時）、「還有 N 項 →」。OmniContext 自己的提醒（Extension 沒 heartbeat、秘書沒有每日排程）留在完整清單裡，只在沒有別的可看時才佔焦點。
- **記得 · 秘書為你挑的一則**：依固定順序——與焦點專案有關的決定／筆記 → 最近一天的工作誌 → 你釘選的 → 你最近記下的；卡上寫「為什麼挑這則」。挑不到就說沒有。
- **💬 問秘書**：每張卡一鍵把一句帶脈絡的問題預填到交辦框（不會自動送出），例如「關於『uavMonitor 落後遠端 2 個 commit』：遠端有新 commit。我現在該怎麼處理？」。
- **詳情 chip**：全部提案（N）、記憶區（N）會展開下方收合的面板；進行中專案（N）、Git 同步中心、知識庫、摘要與統計直接跳分頁。

**收起來的東西沒有少**：「TODAY · 全部提案（詳情）」收合在桌面底部，「記憶區」收合在交辦框裡面，一按就開，展開狀態會記住（localStorage）。

**版面**：01 只剩兩塊——左邊秘書桌面（問候併在最上面，今天／近 2 小時切換與 ↻ 在桌面標題列），右邊交辦與提問（記憶區併在下面）——在 1080p 與 1440×900 不用捲動就看完。原本右欄的「今日統計」「FOCUS NOW」搬到 05 摘要與統計最上方，「DATA TRUST」搬到 06 系統設定 → 系統健康。

**右邊的交辦與提問**：直接向小秘書提問或交辦，走本機知識庫 RAG 與所選模型；**它和 02 知識庫分頁是同一條對話**，完整引用卡片、對話歷史與索引管理在那邊。標題列徽章顯示 `RULES`（純規則）或 `LLM · 供應商`（已啟用 LLM 註解）。

**一天離開首頁幾次**：桌面底下有一行「今天離開首頁 N 次 · 昨天 M 次」——這是這個設計的尺：如果 01 真的夠當首頁，這個數字幾天後應該變少。它**只算這個瀏覽器的分頁切換**、只存在瀏覽器裡，不進資料庫。

**邊界**：桌面只重新排列既有的唯讀資料，挑法是確定性的規則、不呼叫 LLM、不寫任何東西；哪一節讀不到，徽章會從 RULES 變 PARTIAL 並列出來，其他節照出。`GET /api/v1/secretary/home`（唯讀）。焦點挑錯，跟其他提案一樣用 snooze 或「偏好：不要提醒 <類型或專案>」。
...
- **沒被採集到的工作不代表沒做**：卡底固定一行說明採集範圍；**郵件目前不在採集範圍**，秘書不會編造「郵件也回了幾封」這類數字。行事曆要先在「系統設定 → 採集來源」加入本機 `.ics`（見下一節），沒加就同樣如實寫「行事曆不在採集範圍」。
...
### 告訴秘書你在乎什麼、想怎麼被講話（ADR-018）

ADR-017 讓秘書**推**出你的主線；這一步讓你可以直接**說**。兩種宣告都是一般的偏好筆記，在對話框或 Telegram 打就好：

- **「偏好：優先：uavMonitor、論文」**——本期優先專案。這些專案的提案（需要 pull、PR、未結事項、被冷落…）都會加分排到同類提案前面，卡上多一句「你把這個專案標為本期優先」。加分刻意大於習慣加權：你說的優先勝過活動算出來的主線；被冷落的優先專案也因此浮上來。
- **「偏好：語氣：簡潔」**（或 **直接**／**溫暖**）——問候卡、晨報開頭與 `/today` 的鼓勵語怎麼講：簡潔＝不講、直接＝一句話只講下一步、溫暖＝原本的情境池。**只改措辭，不改任何數字**；標題與成就清單在三種語氣下完全相同。

**怎麼看現在的宣告**：01 記憶區面板最上面一列會列出你宣告的專案與語氣（`GET /api/v1/secretary/profile`，唯讀）；秘書回答時也會先看到一行「個人檔案（你宣告的）：…」。**要改就再寫一則、要取消就刪掉那則偏好筆記**——沒有專用表單，也沒有第二套資料。

**邊界**：秘書只認你明確寫下的「優先：」「語氣：」，**不從活動、prompt 或對話推斷**你的優先或個性；未知的語氣（「語氣：搞笑」）不套用。專案名比對不分大小寫，但要與 04／提案卡上顯示的專案鍵一致。設定只有一個：`proactive_secretary.profile.priority_boost`（預設 0.2；0 = 只顯示不加分）。

### 每日工作誌：讓小秘書真的知道你每天做了什麼（ADR-012 Addendum A）

在 Antigravity 下「幫我更新同步這個專案」、在儀表板測試、編修論文——這些 OmniContext 都採集到了，但在這之前**秘書的大腦裡沒有留下任何一天的紀錄**（觀察只來自早晨包，講的是秘書自己輸出了什麼）。所以問它「我昨天做了什麼」，它答不出來。

**每日工作誌**補的就是這段：把某一天的活動 reduce 成記憶區的一則觀察。

- **怎麼開**：「系統設定 → 秘書與自動化 → 排程任務」新增 `daily_digest`（`days_back` 預設 1＝昨天整天），或直接按「立即執行」。如果你已經建了**早晨包**排程，它已經是早晨包的第四步，不必另外設定。
- **會寫進去什麼**：一則「YYYY-MM-DD 工作誌」（commit 數與 repo、AI 對話輪數與平台、檔案異動並區分論文文檔與程式、推進的專案、收掉的未結事項、開會場次、前景時間），加上每個有實質活動的專案各一則（例如「2026-09-04 · uavMonitor：2 個 commit、4 輪 AI 對話」）。
- **「重點」那一行**來自當天已經存下來的**時段微摘要**（checkpoint 之後由本機 Ollama 產生的 ≤100 字重點）。**工作誌自己不呼叫任何 LLM**，它只是把既有的東西 reduce 起來。沒有微摘要的那天只會有計數，正文會如實寫「以上只有計數」。
- **秘書會用它**：這些觀察由記憶區自動注入每次對話的 system prompt，所以你可以直接問「我昨天做了什麼」「上週在 uavMonitor 上做了什麼」，回覆下方也會顯示「🧠 參考記憶區 N 筆」。
- **可刪、會過期、不重複**：它就是一般的秘書觀察——記憶區可以一鍵刪除，超過 `secretary_memory.observation_ttl_days` 就不再注入對話；同一天重跑不會寫第二遍。
- **隱私邊界**：**不存你的 prompt 或 AI 回應原文**（ADR-012 的邊界沒有放寬），只存計數與已經壓縮過的時段摘要。要整個關掉：`proactive_secretary.daily_digest.enabled: false`；把 `secretary_memory.enabled` 關掉則一律不寫。

### 模式感知提案：秘書開始注意你的習慣（ADR-017）

有了每日工作誌之後，秘書會**用**它記得的東西。01 的提案清單多了兩種卡，排序也會變：

- **「你近一週有 N 天在工作（uavMonitor、論文），但秘書還沒有每日排程」**——代表你有在用，但早晨包／工作誌沒有在跑，記憶區不會累積。按「📦 建立每日排程」之後這張卡就消失。
- **「X 前一週活躍 4 天，近一週完全沒動」**——沒有未結事項在提醒你、只有模式看得到的那種擱置。卡上可直接產生 X 的 Context Handoff（L0，唯讀），看一眼再決定接續還是明確放下。
- **主線排前面**：近一週活躍 3 天以上的專案，它的「需要 pull」「PR 待處理」等既有提案會多一句「這個專案近 7 天有 N 天在動，是你目前的主線」並排在冷門 repo 前面。

**怎麼算的**：只用 commit／AI 對話／檔案異動依「專案 × 日」的計數；**不讀 prompt 內容、不推測你想做什麼、今天不算**（今天的分母只到現在）。沒歸戶的活動不會被塞進某個專案。`GET /api/v1/secretary/proposals` 的 `inputs.patterns` 會列出近一週各專案的活動天數，可以拿去對 04 統計。

**誤判怎麼辦**：跟其他提案一樣——按 snooze，或在對話框說「偏好：不要提醒 neglected_active_project」（或某個專案名）就永遠不再出現。設定在 `proactive_secretary.patterns`（`enabled`、`lookback_days`、各門檻、`habit_boost: 0` 可單獨關掉加權）。

### 每週回顧：說的 vs 做的（ADR-020）

你在偏好筆記宣告了「優先：論文」，秘書也知道你這週在 uavMonitor 上動了五天、論文一天。從這一版起它會把兩件事放在一起講：

- **記憶區多一則「2026-W37 回顧（09-07～09-13）」**：7 天裡幾天有活動、依活躍天數列出專案、你宣告的優先各幾天並寫「說的和做的一致／不一致」、每日工作誌寫了幾天。早晨包每天會補上一個完整週的回顧（同一週只寫一次），也可在「排程任務」新增 `weekly_review` 或按立即執行（`weeks_back` 1–4 回看更早的週）。它是一般的秘書觀察：可刪、會過期，問「上週我做了什麼」秘書會用到。
- **不一致就多一張提案**：「你說『論文』優先，上週它只有 1 天在動、uavMonitor 有 5 天」。因為它是你宣告的優先，會被優先加分，多半直接成為 01 桌面的焦點。卡上的建議是二選一：下週真的把時間排給它（看一眼 Handoff 接續），或改宣告——在對話框打「偏好：優先：…」換成你真正在做的。
- **什麼時候不算偏移**：整週都很安靜（沒有別的專案 ≥ 3 天）不算；兩個宣告的優先一個做了、一個沒做也不算——你在做你說的事之一。宣告的名字對不到任何活動時，回顧會如實寫「沒有任何活動歸到這個名字」，可能是名字與 04 顯示的專案鍵不同。
- **邊界**：只用 commit／AI 對話／檔案異動的（專案 × 日）計數與你的偏好筆記；**不讀 prompt、不呼叫 LLM、不推測為什麼**——「不一致」只是兩個數字放在一起，判斷由你來。偏移卡跟其他提案一樣可以 snooze 或「偏好：不要提醒 priority_drift」。

```yaml
proactive_secretary:
  weekly_review:
    enabled: true
    drift_max_days: 1         # 宣告優先的專案上週活躍 ≤ 幾天算「說了沒做」
    drift_min_other_days: 3   # 且同週有別的專案 ≥ 幾天才算「時間去了別處」
```

### 文件落後了：那句你常打的指令（ADR-021）
...
### 本機行事曆（.ics，唯讀，ADR-015）

讓小秘書知道你今天有哪些會，但**不連任何雲端**：

1. 把行事曆匯出或同步成 `.ics` 放在本機——Outlook「儲存行事曆」、Google 日曆「匯出」或訂閱網址由行事曆軟體端同步到本機資料夾、Apple 行事曆匯出、Thunderbird 本機快取都可以。
2. 「系統設定 → 🧲 採集來源 → 📅 本機行事曆」加入 `.ics` 檔或含 `.ics` 的資料夾（資料夾只收第一層），儲存並套用。
3. 幾秒後「系統健康」的「行事曆（.ics）」會顯示「N 個檔 · 視野內 M 筆」；01 首頁今日面板多一行「📅 今天 N 場行程，下一場 14:00 …」，問候卡多「開了 N 場會（M 分鐘）」，隔天晨報多一段「📅 今日行程」。

**邊界**：只讀時間／標題／地點／狀態；DESCRIPTION、與會者、連結、提醒一律不落地，會議標題也不會送進任何 LLM 或 RAG 索引。`store_titles: false` 時連標題與地點都不存，只留時間（共用機器或敏感行事曆用）。重複行程只展開到未來 `horizon_days`（預設 30 天，另含過去 7 天供回顧）；取消（`STATUS:CANCELLED`）的不顯示、暫定的標「暫定」。沒有設定任何路徑就等於停用，不會出現「尚無紀錄」的假警報。`GET /api/v1/calendar/agenda?date=YYYY-MM-DD` 可查任一天，回應附 `claim_boundary`。

### 會議秘書：會後把逐字稿變成紀錄與候選待辦（ADR-022）

秘書**看不到也聽不到你的線上會議**，也不會去讀 Teams 視窗或錄音。它做的是**會後**那段：

1. 在 `meetings.transcript_dir` 設一個資料夾（沒設就是停用）。Teams 開了轉錄的會議，會後把逐字稿（`.vtt`／`.docx`，也接受 `.txt`／`.md`）下載或同步到那裡。
2. 「系統設定 → 排程任務」新增 **`meeting_notes`**（L0 唯讀），或直接對它按「立即執行」。每份還沒整理過的逐字稿會變成**一則記憶區觀察**：配對到的會議標題（只用時間配對，配不到就寫「未配對到行事曆事件」）、發言段數與講者數、三到五句摘要、**候選待辦**。
3. 01 的提案卡出現「會議紀錄：…」，每條候選待辦旁邊是「**加入未結事項**」與「**忽略**」。**只有你點了才會進未結事項**——秘書自己永遠不寫。沒點的不進任何計數、不進週回顧。
4. 會議進行中（行事曆有正在進行的行程 **且** Teams／Zoom 在前景），桌面多一行「📞 會議中：<標題>（還剩 N 分）」。只有其一時會如實說差異，例如「行事曆說現在是 X，但沒看到會議軟體在前景」。
5. 剛結束的會議 15 分鐘後仍沒有新逐字稿，會提一張卡提醒你去下載；有檔案落地就不再提。

**摘要的 provider 預設 `ollama`（全本機）**。逐字稿裡有其他與會者的發言，所以改成雲端 provider（gemini／anthropic／openai）等於把他們的話送到那家供應商——觀察正文會明白寫「（雲端：與會者的發言曾送往該供應商）」。prompt 與回應原文一律不落地。

摘要失敗不會讓紀錄消失：連不上 provider 時觀察會寫「provider 未回覆摘要：…」並建議跑 `python main.py llm-test`；摘要出現逐字稿沒有的數字時整段丟掉（摘要與候選待辦一起），只留逐字稿的可回溯計數。

**要不要讓會議內容進知識庫，是你的決定**：把那個資料夾加進「02 知識庫」才會被索引（`.vtt` 已在支援清單）。摘要與觀察不依賴索引。

```powershell
Invoke-RestMethod "http://127.0.0.1:8765/api/v1/secretary/meetings" | ConvertTo-Json -Depth 3
```

**不做的事**：錄音、即時字幕／翻譯、讀會議軟體視窗內容、呼叫 Teams／Graph API、自動下載逐字稿、保存與會者名單、會前提醒。即時那一層要另一份 ADR 並過 ADR-022 D6 的五道門。
...
### 在手機上用小秘書（Telegram 對話，ADR-013，預設關閉）
