...
7. Extension `1.3.0` 每分鐘以 `chrome.alarms` 送出經 ingest token 驗證的 heartbeat；server 只保存 instance ID、版本、content-ready platforms、capture status、error code、queue size 與時間，不保存 token、URL、Prompt 或 Response。
