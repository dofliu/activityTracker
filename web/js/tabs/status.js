...
export function initControls() {
  $("btn-toggle-monitor").addEventListener("click", async () => {
    try {
      await postJSON(state.feed.isMonitoring ? "/api/v1/control/stop" : "/api/v1/control/start");
      refreshStatus();
  ...
  document.querySelectorAll(".filters .chip").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      state.feed.activeFilter = btn.dataset.filter;
      refreshFeed();
    });
  });
}
...
export async function refreshStatus() {
  try {
    ...
    state.feed.isMonitoring = data.is_running;
    const monitoringState = data.monitoring_state || (state.feed.isMonitoring ? "healthy" : "stopped");

    const pill = $("status-pill");
    pill.className = "pill " + (monitoringState === "degraded" ? "pill-warn" : state.feed.isMonitoring ? "pill-on" : "pill-off");
    $("status-text").textContent = monitoringState === "degraded"
      ? t("status_degraded")
      : state.feed.isMonitoring ? t("status_monitoring") : t("status_paused");
    $("btn-toggle-monitor").textContent = state.feed.isMonitoring ? t("btn_pause_monitor") : t("btn_start_monitor");
...
export function renderUsagePanel(data) {
  ...
  const goalText = state.ui.currentLang === "zh-TW"
  ...
  $("usage-boundary").textContent = state.ui.currentLang === "zh-TW"
  ...
  $("usage-interface-list").innerHTML = rows.length ? rows.map(item => `
    ...
    </div>`).join("") : `<div class="placeholder">${state.ui.currentLang === "zh-TW" ? "今日尚無已觀察到的介面使用資料。" : "No observed interface usage today."}</div>`;
}

export function renderUsagePanelError() {
  $("usage-coverage").className = "trust broken";
  $("usage-coverage").textContent = "UNAVAILABLE";
  $("usage-interface-list").innerHTML = `<div class="placeholder">${state.ui.currentLang === "zh-TW" ? "無法載入使用時間。" : "Unable to load usage data."}</div>`;
}

// 參數原本叫 `state`，把 `core/state.js` 匯入的共享 `state` 遮蔽掉了：`state.ui.currentLang`
// 讀的是一個字串（"observed" 之類）的 `.currentLang`，永遠 undefined，所以中文介面一直拿到
// 英文標籤。同一個檔案裡的 `renderCaptureCoverage` 用的就是共享的 `state.ui.currentLang`，可見
// 這裡是不小心遮到，不是刻意。
export function captureStateLabel(captureState) {
  const labels = state.ui.currentLang === "zh-TW" ? {
...
export function renderCaptureCoverage(data, extensionData) {
  ...
  $("capture-extension-summary").textContent = extension
    ...
    : (state.ui.currentLang === "zh-TW" ? "Browser Extension 狀態目前無法取得。" : "Browser Extension status is unavailable.");
  ...
  $("capture-coverage-boundary").textContent = state.ui.currentLang === "zh-TW"
...
export function renderCaptureCoverageError() {
  ...
  $("capture-extension-summary").textContent = state.ui.currentLang === "zh-TW" ? "採集狀態暫時無法取得。" : "Capture status is unavailable.";
  $("capture-coverage-list").innerHTML = `<div class="placeholder">${state.ui.currentLang === "zh-TW" ? "無法取得採集 coverage。" : "Capture coverage is unavailable."}</div>`;
}

export function renderStats(m) {
  const items = [
    { tag: "AI TURNS", value: m.ai_prompts_count, label: "Claude / Codex / Web", trust: "ok" },
    { tag: "FILES", value: m.file_events_count, label: state.ui.currentLang === "zh-TW" ? "論文與檔案異動" : "Paper & File Events", trust: "ok" },
    { tag: "COMMITS", value: m.git_commits_count, label: state.ui.currentLang === "zh-TW" ? "Git commits 提交" : "Git Commits", trust: "ok" },
    { tag: "FOCUS", value: m.window_events_count, label: state.ui.currentLang === "zh-TW" ? "視窗焦點切換" : "Window Focus", trust: "ok" },
    { tag: "STREAMS", value: state.projects.cache.length, label: state.ui.currentLang === "zh-TW" ? "進行中工作" : "Active Workstreams", trust: "ok" }
  ];
...
export function renderCollectors(w, lastEvents = {}, health = {}, diagnostics = {}) {
  ...
  const labelMap = {
    healthy: state.ui.currentLang === "zh-TW" ? "運作中" : "Active",
    idle: state.ui.currentLang === "zh-TW" ? "待命中" : "Idle",
    degraded: state.ui.currentLang === "zh-TW" ? "部分採集異常" : "Degraded",
    stopped: state.ui.currentLang === "zh-TW" ? "已停止" : "Stopped",
    stale: state.ui.currentLang === "zh-TW" ? "無有效資料" : "Stale",
    disabled: state.ui.currentLang === "zh-TW" ? "已停用" : "Disabled"
  };
  ...
  $("watchers-grid").innerHTML = items.map(it => {
    ...
    if (it.last) {
      ...
      lastTimeStr = `<span class="mono-mini" style="font-size:10px; opacity:0.85; display:block; margin-top:2px; color:var(--text-dim);">${state.ui.currentLang === "zh-TW" ? "最後寫入" : "Last"}: ${timePart}</span>`;
    } else if (it.on && it.key !== "scheduler") {
      lastTimeStr = `<span class="mono-mini" style="font-size:10px; opacity:0.85; display:block; margin-top:2px; color:var(--danger, #ef4444); font-weight:600;">${state.ui.currentLang === "zh-TW" ? "尚無紀錄 (待排查)" : "No data (Pending)"}</span>`;
    }
    ...
    if (it.key === "window_watcher" && it.d && ["unavailable", "error"].includes(it.d.state)) {
      diagnosticText = state.ui.currentLang === "zh-TW"
    ...
    if (it.key === "calendar_watcher" && it.d) {
      if (it.d.state === "unconfigured") {
        diagnosticText = state.ui.currentLang === "zh-TW" ? "尚未設定 .ics 路徑（系統設定 → 採集來源）" : "No .ics paths configured (Settings → Sources)";
      } else if (Number(it.d.degraded_sources_count || 0) > 0) {
        const names = (it.d.degraded_sources || []).map(s => s.source_name).join(", ");
        diagnosticText = `${state.ui.currentLang === "zh-TW" ? "來源錯誤" : "Source error"}: ${names}`;
    ...
    if (it.key === "agent_log_watcher" && it.d && it.d.sources) {
      ...
      if (failed.length) diagnosticText = `${state.ui.currentLang === "zh-TW" ? "來源錯誤" : "Source error"}: ${failed.join(", ")}`;
