...
* **小秘書會主動說話（2026-09-04）**：01 分頁最上方的「🤗 小秘書的話」卡，說明今天或近 2 小時做了什麼（commit、PR、AI 對話、論文與程式檔案、推進的專案、收掉的事項）再接一句貼心的鼓勵。每個數字都可回溯到資料表，沒被採集到的工作不代表沒做——郵件與行事曆不在採集範圍，卡上寫明；鼓勵語由規則挑選、同一天不跳動，LLM 潤飾預設關閉且不得多出統計裡沒有的數字。
